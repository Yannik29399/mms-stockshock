...
export class DiscordNotifier implements Notifier {
    ...
    private readonly checkOnlineStatus: boolean = false;
    private readonly checkInAssortment: boolean = false;
    private readonly announceCookies: boolean = true;
    private readonly shoppingCartAlerts: boolean = false;
    ...
    constructor(store: Store, storeConfig: StoreConfiguration, logger: Logger) {
        ...
        this.announceCookies = storeConfig.announce_cookies ?? false;
        this.shoppingCartAlerts = storeConfig.shopping_cart_alerts = false;
        this.showCookiesAmount = storeConfig.show_cookies_amount ?? false;
        this.showMagicianLink = storeConfig.show_magician_link ?? true;
        this.checkOnlineStatus = storeConfig.check_online_status = false;
        this.checkInAssortment = storeConfig.check_in_assortment = false;
        this.showThumbnails = storeConfig.show_thumbnails ?? true;
    ...
    async notifyStock(item?: Item, cookiesAmount?: number): Promise<void> {
        ...
        if (this.showMagicianLink) {
                embed.addField("Link", `${this.productHelper.getProductURL(item, this.store, this.replacements, true)}`);
            }
    ...
    private createEmbed(item: Item) {
        ...
        embed.setFooter(`Alerts by Datenheim • Links may be affiliate links`);
        if (!item.product) {
            return embed;
        }
